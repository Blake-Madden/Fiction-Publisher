...
for i in $(ls _site -I *Amazon*); do
    mkdir Books/"${i%%.*}"
    pandoc --toc-depth=1 --template=Pandoc/templates/custom-epub.html --epub-stylesheet=Pandoc/css/style.css -o Books/"${i%%.*}"/"${i%%.*}".epub _site/$i
done

# Create mobi for every book in _site
for i in $(ls _site | grep Amazon); do
    pandoc --toc-depth=1 --template=Pandoc/templates/amazon-epub.html --epub-stylesheet=Pandoc/css/style.css -o Books/"${i%-Amazon.md}"/"${i%%.*}".epub _site/$i
